...
class LobsterR1GVFs(GeneralValueFunction):
    """
    Single GVF with policy that always goes left.
    Everything is done with batches!
    """
    def __init__(self,
                 n_actions: int,
                 gamma: float):
        super(LobsterR1GVFs, self).__init__(n_actions)
        self.action_idxes = np.array([0], dtype=int)
        self.gamma = gamma
    ...
    def termination(self, obs: np.ndarray) -> np.ndarray:
        # return np.zeros((obs.shape[0], 2)) + self.gamma
        return obs[:, [5]] * self.gamma

    def policy(self, state: np.ndarray) -> np.ndarray:
        pis = np.zeros((state.shape[0], 1, self.n_actions))
        pis[:, np.arange(pis.shape[1]), 0] = 1
        return pis
    ...
    def impt_sampling_ratio(self, state: np.ndarray, b: np.ndarray, actions: np.ndarray) -> np.ndarray:
        ...
        actions: batch
        """
        all_is_ratios = super(LobsterR1GVFs, self).all_impt_sampling_ratios(state, b)
        actions_mask = np.zeros((actions.shape[0], self.n_actions), dtype=bool)
        actions_mask[np.arange(actions.shape[0]), actions] = True
        actions_mask_n_gvfs_repeat = np.expand_dims(actions_mask, 1).repeat(all_is_ratios.shape[1], 1)
        flattened_all_is_ratios = all_is_ratios[actions_mask_n_gvfs_repeat]
...
class LobsterR2GVFs(GeneralValueFunction):
    """
    Single GVF with policy that always goes left.
    Everything is done with batches!
    """
    def __init__(self,
                 n_actions: int,
                 gamma: float):
        super(LobsterR2GVFs, self).__init__(n_actions)
        self.action_idxes = np.array([1], dtype=int)
        self.gamma = gamma
    ...
    def termination(self, obs: np.ndarray) -> np.ndarray:
        # return np.zeros((obs.shape[0], 2)) + self.gamma
        return obs[:, [8]] * self.gamma

    def policy(self, state: np.ndarray) -> np.ndarray:
        pis = np.zeros((state.shape[0], 1, self.n_actions))
        pis[:, np.arange(pis.shape[1]), 1] = 1
        return pis
    ...
    def impt_sampling_ratio(self, state: np.ndarray, b: np.ndarray, actions: np.ndarray) -> np.ndarray:
        ...
        actions: batch
        """
        all_is_ratios = super(LobsterR2GVFs, self).all_impt_sampling_ratios(state, b)
        actions_mask = np.zeros((actions.shape[0], self.n_actions), dtype=bool)
        actions_mask[np.arange(actions.shape[0]), actions] = True
        actions_mask_n_gvfs_repeat = np.expand_dims(actions_mask, 1).repeat(all_is_ratios.shape[1], 1)
        flattened_all_is_ratios = all_is_ratios[actions_mask_n_gvfs_repeat]
