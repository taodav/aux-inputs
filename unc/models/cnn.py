...
def cnn(hidden_size: int, actions: int, x: np.ndarray):
    ...
    convs = hk.Sequential([
        Conv2D(output_channels=32, kernel_shape=5, stride=1, padding="VALID",
               w_init=init, b_init=b_init),
        jax.nn.relu,
        Conv2D(output_channels=hidden_size, kernel_shape=4, stride=1, padding="VALID",
               w_init=init, b_init=b_init),
        jax.nn.relu,
        Conv2D(output_channels=hidden_size, kernel_shape=2, stride=1, padding="VALID",
               w_init=init, b_init=b_init),
        jax.nn.relu
    ])
