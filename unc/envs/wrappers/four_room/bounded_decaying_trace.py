...
class BoundedDecayingTraceObservationWrapper(FourRoomWrapper):
    """
    ...
    def __init__(self, env: Union[FourRoom, FourRoomWrapper], decay: float = 0.9):
        super(BoundedDecayingTraceObservationWrapper, self).__init__(env)

        self.decay = decay
        self.max_obs = np.ones(self.n_rewards)
